...
runGA <- function(noRuns = 30, problem = "feature"){
  ...
  if (problem == "feature"){
    maxGenerations <<- 50    #<<- makes it a global variable. So it will be visible to other functions e.g. monitor()
    popSize = 200
    pcrossover = 0.8
    pmutation = 0.1
    type = "binary"
    ...
    xx <- data[,-ncol(data)]
    yy <- data[,ncol(data)]
    fitness = class_fit              #fitness function defined in feature-selection.R
  }
  else if (problem == "tsp"){
    maxGenerations <<- 50
  ...
  for (i in 1:noRuns){
    ...
    if (problem == "feature")
      GA <- ga(type=type, fitness = fitness, xx=xx, yy=yy, nBits = ncol(xx), 
               ...
               pcrossover = pcrossover,crossover = gabin_uCrossover, pmutation = pmutation, 
  ...
  return (list(resultsMatrix,bestFitness))
}
